...
//text color
final int BLUE = 0, WHITE = 1, YELLOW = 2, ORANGE = 3, RED=4 ;
int colorState=0;
...
void initCabbages() {
  ...
  for (int i = 0; i < cabbageX.length; i++) {
    ...
    cabbageY[i]=floor(random(0+i*4, 4+i*4))*SOIL_SIZE;
  ...
  case GAME_RUN: // In-Game
    ...
    for (int i = 0; i < cabbageX.length; i++) {
      // Requirement #3: Use boolean isHit(...) to detect collision
        ...
        while (playerHealth < PLAYER_MAX_HEALTH && isHit(playerX, playerY,SOIL_SIZE,SOIL_SIZE,cabbageX[i], cabbageY[i],SOIL_SIZE,SOIL_SIZE)==true) {
          cabbageX[i]=2000;
          cabbageY[i]=2000;
     
          if(cabbageX[i]==2000 && cabbageY[i]==2000){
           playerHealth +=1;
          }
...
void addTime(float seconds) {	
  gameTimer+=seconds; 				// Requirement #2
}
...
int getEnemyIndexByRow(int Row) {				// Requirement #6
  ...
  for (int i=0; i<soldierY.length; i++) {
    //println(soldierY[i]/SOIL_SIZE + " " + str(Row+5));
    if (int(soldierY[i]/SOIL_SIZE) == (Row+5)) {
      soldierNumber=i;
      //println("soldier:" + soldierNumber);
    } 
